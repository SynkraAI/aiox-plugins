...
export const ANALYZER_LIMITS = Object.freeze([
  "An MCP server is a RUNTIME-RESOLVED POINTER, not an inspectable artifact: the manifest supplies `{command, args}` (see the product repo's crates/aiox-core/src/mcp.rs:68, typically `npx <package>`), resolved against a registry AIOX does not control. This analysis covers the POINTER; it has never opened the TARGET. The `npx` target is NEVER inspected, downloaded or executed here, so nothing below constrains what that package does once it runs.",
