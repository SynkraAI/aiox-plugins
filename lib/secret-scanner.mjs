...
export function tarMemberTable(tarPath) {
  ...
  const members = names.map((raw, i) => ({
    ...
    // Column 1 of a tar long listing is the type: `-`/`0` regular, `d` directory, `l` symlink,
    // `h` hardlink, `p` FIFO, `s` socket, `c`/`b` device.
    type: verbose[i][0],
  }));
  return { aligned: true, members, counts: { names: names.length, verbose: verbose.length } };
}

// Splits the member table into "ordinary files whose bytes we can go read" and "everything else,
// which is unscannable". Directories are structural, carry no bytes, and are present in every normal
// artifact — refusing them would refuse every package, so they are excluded from both lists.
export function classifyMembers(table) {
  ...
  const content = table.members.filter((m) => m.type !== "d" && !m.raw_path.endsWith("/"));
  const seen = new Map();
