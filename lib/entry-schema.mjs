...
export function validateEntryShape(entry) {
  ...
  // story 055.W4.2 (D17/D20(4)) — the derived-capability block. Two things are checked, and the
  // second is the one that matters: `self_declared` must be LITERALLY false. The block is written
  // by the AIOX-side analyzer at publish time; an entry arriving with `self_declared: true` (or
  // with the flag missing) is either hand-edited or produced by something that is not the
  // analyzer, and in both cases the value cannot be shown to a user as derived provenance.
  if (entry.capabilities !== undefined) {
    const c = entry.capabilities;
    if (typeof c !== "object" || c === null || Array.isArray(c)) {
      errors.push(`${tag}: capabilities must be an object produced by the capability analyzer (D20(4))`);
    } else {
      if (c.self_declared !== false)
        errors.push(
          `${tag}: capabilities.self_declared must be literally false (D17/AC3) — capabilities are DERIVED by AIOX-side static analysis, never asserted by the publisher; an entry claiming otherwise cannot be displayed as derived provenance`,
        );
      if (!Array.isArray(c.union))
        errors.push(`${tag}: capabilities.union must be an array (may be empty) — the derived capability set`);
      if (!Array.isArray(c.limits) || c.limits.length === 0)
        errors.push(
          `${tag}: capabilities.limits must be a non-empty array (D17/AC5) — an analysis displayed without its blind spots lies by omission; the MCP-pointer limitation in particular must travel with the result`,
        );
    }
  }
