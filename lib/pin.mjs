...
//
// ── WHY AMBIGUITY IS A REFUSAL, NOT A "PICK THE LAST ONE" ────────────────────────────────────────
//
// If one index somehow carried two entries for the same `plugin_id@version` with different digests,
// any tie-break rule (first, last, highest) would make the resolved BYTES depend on entry ORDER —
// i.e. on how the file was edited — which is exactly the silent-artifact-substitution D24(b) exists
// to prevent. Refusing is the only answer that keeps the pin meaningful.
...
export function resolvePin(indexData, pin) {
  ...
  if (digests.length > 1) {
    throw new Error(
      `pin "${pin}" — REFUSED: ${exact.length} entries share this plugin_id@version with ${digests.length} DIFFERENT digests (${digests.join(", ")}). A pin whose bytes depend on entry order is not a pin; the index is corrupt and must be fixed, not tie-broken.`,
    );
  }
  ...
  return {
    ...
    tiers: e.tiers ?? [],
    digest: { algorithm: e.digest.algorithm ?? "sha256", value: e.digest.value },
