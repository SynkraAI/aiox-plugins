...
| Path convention (production) | `plugins/<plugin_id>/<version>/<sha256>.tar.gz` — content-addressed by digest, so the same bytes are never silently swapped under an existing pointer. |
